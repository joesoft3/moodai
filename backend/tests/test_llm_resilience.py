...
# ---------- sibling-bucket helper ----------

def test_sibling_swaps_both_ways(monkeypatch):
    _env(monkeypatch)
    assert llm._fallback_sibling("gemini", "gemini-2.5-flash") == "gemini-2.5-pro"
    assert llm._fallback_sibling("gemini", "gemini-2.5-pro") == "gemini-2.5-flash"


def test_sibling_never_leaves_fallback_provider(monkeypatch):
    _env(monkeypatch)
    assert llm._fallback_sibling("xai", "grok-4") is None
    assert llm._fallback_sibling(None, "grok-4") is None
    assert llm._fallback_sibling("openai", "gpt-4o") is None


def test_sibling_respects_kill_switch(monkeypatch):
    _env(monkeypatch)
    monkeypatch.setattr(settings, "LLM_FALLBACK_429_SWAP", False)
    assert llm._fallback_sibling("gemini", "gemini-2.5-flash") is None
