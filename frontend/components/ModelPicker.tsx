...
export default function ModelPicker({
  ...
  return (
    <div className="border-t border-line bg-panel/60 backdrop-blur px-2 sm:px-3 pt-2 compact-v">
      <div className="max-w-3xl xl:max-w-4xl 2xl:max-w-5xl mx-auto flex items-center gap-1.5 flex-wrap">
        <div className="flex items-center gap-1 rounded-full bg-base border border-line p-1">
          {MODEL_OPTIONS.map((o) => (
            <button
              key={o.id}
        ...
        {arenaMode && (
          <select
            ...
            className="rounded-full bg-base border border-line text-[11px] text-gray-400 px-2 py-1.5 outline-none focus:border-accent/50"
          >
            {ARENA_EXTRAS.map((v) => (
